 ...
 * }
 */
function dima_export_wp( $args = array() ) {
	global $wpdb, $post;
	 ...
	 */
	$filename = apply_filters( 'dima_export_wp_filename', $wp_filename, $sitename, $date );
	...
	if ( in_array( $args['content'], array( 'post', 'page', 'attachment' ), true ) ) {
		...
		if ( $args['page_id'] ) {
			$where .= $wpdb->prepare( " AND {$wpdb->posts}.ID >= %s", intval( $args['page_id'] ) );
		}
